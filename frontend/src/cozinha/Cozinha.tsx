...
function Cozinha() {
  // Estado para armazenar o status dos dispositivos da cozinha
  ...
  useEffect(() => {
    async function fetchStatus() {
      try {
        const data = await getCozinhaStatus(); // Função que obtém o status da cozinha
  ...
  const handleControl = async (deviceId: string, action: 'on' | 'off') => {
    try {
      await controlDeviceCozinha(deviceId, action); // Envia o comando para o backend
